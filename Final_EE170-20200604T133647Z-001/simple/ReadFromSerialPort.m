...
global s1;
s1=serial('COM6','Baudrate',115200);
...
while(j<5000)
    ...
    accX(j)=getSignedReading( reading(1), accX_offset);
    accY(j)=getSignedReading( reading(2), accY_offset);
    accZ(j)=getSignedReading( reading(3), accZ_offset);
    gyroX(j)=getSignedReading( reading(4), gyroX_offset);
    gyroY(j)=getSignedReading( reading(5), gyroY_offset);
    gyroZ(j)=getSignedReading( reading(6), gyroZ_offset);
    x(j)=j;
    ...
    %use atan2 so that rho&phi rang e[-180,180], theta range[0,180]
